...
const addSecretHelper = () => {
	...
	menu.insertAdjacentHTML("beforeend", buildSecretMenuButton(name.value));
	document.getElementById("slackPM_select_" + name.value).addEventListener("click", () => {setSecret(name.value)});
	renderSecretList(getSecrets());
	name.value = "";
	value.value = "";
};
